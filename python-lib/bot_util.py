...
def seed_move(seed_dir, target_dir, keyword):
    ...
    elif str.isdigit(seed_dir) and int(seed_dir) > 0 and int(seed_dir) <= 100:
...
MESSAGE_DICT['usage_mhrize_weapon_expected_value_calc'] = 'つかいかた(´・ω・`)\n' \
                                              ...
                                              '属性会心スキルとかは考慮しねえのら(｀・ω・´)\n' \
                                              'モンハンはあそびじゃねえのら(｀・ω・´)'
MESSAGE_DICT['usage_radio_record_reserver'] = 'つかいかた(´・ω・`)\n' \
                                              'rrr [年] [月] [日] [時] [分] [録音時間(秒)] [番組名] [放送局] (映像フラグ)\n'\
                                              'すいれん.tvにろくおんをおねがいする余(｀・ω・´)\n' \
                                              '放送局名は「文化放送」「ニッポン放送」とかの名前でも「QRR」とか「LFR」とかの放送局コードでもいい余(｀・ω・´)\n' \
                                              '放送局名にagqrといれると文化放送 超A&Gの録音をする余(｀・ω・´)\n' \
                                              '映像フラグはagqrのときに指定すると動画を取得する余(｀・ω・´)\n' \
                                              '[年]にl っていれると予約のリストをかえす余(｀・ω・´)\n' \
                                              '[年]にd [月]に削除したい予約番号をいれると予約が削除できる余(｀・ω・´)\n'
MESSAGE_DICT['usage_dice_bot'] = '[dice bot] 使い方\n' \
                                 'dice [(振るダイスの個数)d(ダイスの面数)]'
